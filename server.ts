...
import * as save from './save_offline'
import { play } from './play'
import * as fs from 'fs'
import express from 'express'
import bodyParser from 'body-parser'
import cors from 'cors'
import * as sao from "./ibmSTT" // Sound Audio Oupter
...
// 		fs.readdirSync(scriptPath, readDir => {
// 			if (err) {
...
//
async function tryLoadScript(scriptName) {
	return new Promise((resolve, reject) => {
...
// else break
// }
var script =  tryLoadScript(scriptName).then((result) => {return result;}).catch((err) => {return err;})
