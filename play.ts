...
import {outputObj} from "./playFunc"
...
export function play(input, profile, scriptObj) {
    ...
    if (input == "") {
        ...
        outputMap = updateOutput(outputMap ,displayCustom(stage, chapterStory, player, vars))
        ...
        if (result.output.length > 0) {
            ...
            outputMap = updateOutput(outputMap,displayCustom(stage, result.output.join('\n'), player, vars))
        }
        if (result.output.length == 0 || result.chapter != chapter) {
            ...
            outputMap = updateOutput(outputMap,displayCustom(stageToShow, chapterStory, player, vars))
        }
    }
